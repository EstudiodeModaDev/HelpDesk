  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!Tickets) {
        ...
        const created = await Tickets.createTicket(payloadTicket);
        createdId = created?.data?.ID ?? "";
              
        const payloadLog: LogDTO = {
          seguimientos_solvi_actor: state.solicitante ?? "",
          seguimientos_solvi_correo_actor: state.CorreoTercero ?? "",
          seguimientos_solvi_descripcion: `Se ha creado un usuario POS para ${state.solicitante ?? ""}, se enviarán las credenciales de forma interna`,
          seguimientos_solvi_tipo_de_accion: "Solucion",
          seguimientos_solvi_id_ticket: Number(createdId),
