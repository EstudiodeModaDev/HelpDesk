...
export function useDetallado(TicketsSvc: TicketsRepository) {
  ...
  const buildFilterTickets = React.useCallback((): filterTickets => {
    return { range, };
  }, [range?.from, range?.to]);
  ...
  const obtenerConteoUltimosMeses = React.useCallback(
    async (months: number): Promise<ConteoMes[]> => {
      ...
      try {
        ...
        const res = await TicketsSvc.loadTickets({range: {from: start.toString(), to: endExclusive.toString()}});
