...
export function useSolicitudesRed(TicketSvc: TicketsRepository) {
    ...
    async (e?: React.FormEvent) => {
      ...
      try {
        ...
        if (flow.createdTicket != null) {
          await TicketSvc.updateTicket(String(flow.createdTicket), { FechaMaxima: toGraphDateTime(fechaSolucion) });
        }
...
export function useSolicitudServicios(TicketSvc: TicketsRepository) {
  const { account } = useAuth();
...
export function usePermisosERP(TicketSvc: TicketsRepository) {
...
// ── Hook principal ───────────────────────────────────────────────────────
export function usePermisosNavegacion(TicketSvc: TicketsRepository) {
