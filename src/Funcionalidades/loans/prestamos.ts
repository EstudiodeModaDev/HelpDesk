  ...
  const finalizeLoan = async (loan: prestamos, continuar: boolean) => {
    try {
      ...
      const descripcionTicket = `
        Se ha cerrado el ticket ${escapeHTML(loan.IdTicket ?? "")} asociado al préstamo de equipo.<br><br>

        <strong>El equipo fue recibido por:</strong> ${escapeHTML(account?.name ?? "")}<br>
        <strong>Estado de devolución:</strong> ${continuar ? "Buen estado" : "Mal estado"}<br><br>

        <strong>Estado en el que se entrega el dispositivo:</strong><br>
        ${tablaEntregaHtml}<br><br>

        <strong>Estado en el que se devuelve el dispositivo:</strong><br>
        ${tablaDevolucionHtml}
      `.trim();

      await logs?.createLog({
        seguimientos_solvi_actor: "Sistema",
        seguimientos_solvi_descripcion: descripcionTicket,
...
export function usePruebas() {
  ...
  const createAllPruebas = async (prestamoId: string, deviceid: string) => {
    setSubmitting(true);
    try {
      const pruebasLoaded = await loadDeviceTests(deviceid);
      ...
      console.error("Error en createAllPruebas:", err);
    } finally {
      setSubmitting(false);
    }
  };
...
export function usePruebasDispositivos() {
  ...
  const assignTest = async (deviceId: string, testId: string) => {
    await pruebasDispositivo.create({ Title: deviceId, IdPrueba: testId });
