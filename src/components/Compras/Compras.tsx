...
export default function CompraFormulario({ onSubmit, initial }: Props) {
  ...
  function validate(): boolean {
    ...
    if (state.cargarA === "Marca") {
      if (totalPct !== 100) e.marcasPct = "El total de porcentajes debe ser 100%.";
    }
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (!validate()) return;
    onSubmit?.(state);
    // podrías limpiar si quieres:
    // setState((s) => ({ ...s, productoServicio: "" }));
  }

  // si se cambia a "CO", fuerza los % a 0 para evitar confusiones
  React.useEffect(() => {
    if (state.cargarA === "CO") {
      setState((s) => ({
        ...s,
  ...
  return (
    <form className="invoice-grid" onSubmit={handleSubmit}>
      {/* Fila 1 */}
      <div className="field">
      ...
      <div className="field">
        <label className="label">
          {state.tipoCompra === "Producto"
            ? "Producto"
            : state.tipoCompra === "Servicio"
            ? "Servicio"
            : "Alquiler"}
        </label>
        <input
      ...
      {state.cargarA === "CO" ? (
        ...
        <div className="col-span-full">
          <div className="table-fluid" style={{ padding: 12 }}>
              ...
              {MARCAS.map((m) => (
                <div key={m} className="field">
                  ...
                  <input
                    ...
                    onChange={(e) => setMarcaPct(m, Math.max(0, Math.min(100, Number(e.target.value))))}
                  />
                </div>
              ))}
            </div>
