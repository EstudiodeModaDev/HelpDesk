...
export class CentroCostosService {
  ...
  // ---------- mapping ----------
  private toModel(item: any): CentroCostos {
    const f = item?.fields ?? {};
    return {
  ...
  // ---------- CRUD ----------
  async create(record: Omit<CentroCostos, 'ID'>) {
  ...
  async update(id: string, changed: Partial<Omit<CentroCostos, 'ID'>>) {
    await this.ensureIds()
