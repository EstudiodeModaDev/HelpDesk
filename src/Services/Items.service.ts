...
export class ItemService {
  ...
  // ---------- mapping ----------
  private toModel(item: any): ItemBd {
    const f = item?.fields ?? {};
    return {
        Id: String(f?.id ?? f?.ID ?? f?.Id ?? ''),
        NombreItem: f.NombreItem,
        Title: f.Title,
        Valor: f.Valor
    };
  }
  ...
  // ---------- CRUD ----------
  async create(record: Omit<ItemBd, 'ID'>) {
  ...
  async update(id: string, changed: Partial<Omit<ItemBd, 'ID'>>) {
    await this.ensureIds()
