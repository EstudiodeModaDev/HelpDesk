...
import type { pruebasDispositos } from '../Models/prestamos';
import { ensureIds, esc } from '../utils/Commons';

export class PruebasDispositivoService {
  ...
  constructor(
    ...
    listName = 'Prestamos - PruebasDispostivo'
  ) {
    this.graph = graph;
  ...
  private toModel(item: any): pruebasDispositos {
    const f = item?.fields ?? {};
    return {
  ...
  // ---------- CRUD ----------
  async create(record: Omit<pruebasDispositos, 'ID'>) {
  ...
  async update(id: string, changed: Partial<Omit<pruebasDispositos, 'ID'>>) {
        await this.ensureIds();
  ...
  async getAll(opts?: GetAllOpts): Promise<PageResult<pruebasDispositos>> {
    await this.ensureIds();
  ...
  async getByNextLink(nextLink: string): Promise<PageResult<pruebasDispositos>> {
    return this.fetchPage(nextLink, /*isAbsolute*/ true);
  }

  private async fetchPage(url: string, isAbsolute = false): Promise<PageResult<pruebasDispositos>> {
