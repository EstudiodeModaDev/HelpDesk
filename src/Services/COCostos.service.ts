...
import type { CentroOperativo } from '../Models/CO';
...
export class COService {
  ...
  // ---------- CRUD ----------
  async create(record: Omit<CentroOperativo, 'ID'>) {
  ...
  async update(id: string, changed: Partial<Omit<CentroOperativo, 'ID'>>) {
    await this.ensureIds()
