...
import type { CasosHijosRequeridos } from '../Models/CasosHijosRequeridos';
import type { GetAllOpts } from '../Models/Commons';
import { ensureIds } from '../utils/Commons';

export class CasosHijosRequeridosService {
  ...
  constructor(
    ...
    listName = 'Casos hijos requeridos'     
  ) {
    this.graph = graph;
  ...
  // ---------- mapping ----------
  private toModel(item: any): CasosHijosRequeridos {
    const f = item?.fields ?? {};
    return {
        Id: String(item?.id ?? ''),
        ...
        TipoTienda: f.TipoTienda
    };
  }

  // ---------- CRUD ----------
  async create(record: Omit<CasosHijosRequeridos, 'ID'>) {
  ...
  async update(id: string, changed: Partial<Omit<CasosHijosRequeridos, 'ID'>>) {
    await ensureIds(this.siteId, this.listId, this.graph, this.hostname, this.sitePath, this.listName);;
