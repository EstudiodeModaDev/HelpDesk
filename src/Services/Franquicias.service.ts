...
export class FranquiciasService {
  ...
  // ---------- mapping ----------
  private toModel(item: any): Franquicias {
    const f = item?.fields ?? {};
    return {
        Id: String(item?.id ?? ''),
  ...
  // ---------- CRUD ----------
  async create(record: Omit<Franquicias, 'ID'>) {
  ...
  async update(id: string, changed: Partial<Omit<Franquicias, 'ID'>>) {
    await ensureIds(this.siteId, this.listId, this.graph, this.hostname, this.sitePath, this.listName);;
