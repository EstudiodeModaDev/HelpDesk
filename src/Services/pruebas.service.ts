...
export class PruebasService {
  ...
  private toModel(item: any): pruebasDefinidas {
    const f = item?.fields ?? {};
    return {
  ...
  // ---------- CRUD ----------
  async create(record: Omit<pruebasDefinidas, 'ID'>) {
  ...
  async update(id: string, changed: Partial<Omit<pruebasDefinidas, 'ID'>>) {
        await this.ensureIds();
  ...
  async getAll(opts?: GetAllOpts): Promise<PageResult<pruebasDefinidas>> {
    await this.ensureIds();
  ...
  async getByNextLink(nextLink: string): Promise<PageResult<pruebasDefinidas>> {
    return this.fetchPage(nextLink, /*isAbsolute*/ true);
  }

  private async fetchPage(url: string, isAbsolute = false): Promise<PageResult<pruebasDefinidas>> {
