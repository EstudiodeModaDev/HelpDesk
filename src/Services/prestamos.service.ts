...
export class PrestamosService {
  ...
  private toModel(item: any): prestamos {
    const f = item?.fields ?? {};
    return {
      ...
      FechaPrestamo: f.FechaPrestamo,
      Id_dispositivo: f.Id_dispositivo,
      IdTicket: f.IdTicket,
      Title: f.Title,
      UsuarioRecibe: f.UsuarioRecibe,
  ...
  async update(id: string, changed: Partial<Omit<prestamos, 'ID'>>) {
        await this.ensureIds();
  ...
  async getAll(opts?: GetAllOpts): Promise<PageResult<prestamos>> {
    await this.ensureIds();
  ...
  async getByNextLink(nextLink: string): Promise<PageResult<prestamos>> {
    return this.fetchPage(nextLink, /*isAbsolute*/ true);
  }

  private async fetchPage(url: string, isAbsolute = false): Promise<PageResult<prestamos>> {
